...
CREATE UNIQUE INDEX idx_bookings_code ON bookings(booking_code) WHERE booking_code IS NOT NULL;
...
-- Trạng thái: pending → approved/waiting_transfer/waiting_payment/rejected → exported
-- ─────────────────────────────────────────────────────────────────────────────
CREATE TABLE sales_orders (
    id              UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    created_by      UUID NOT NULL REFERENCES users(id),   -- NV tạo đơn
    branch_id       INT REFERENCES branches(id),
    order_type      VARCHAR(20) NOT NULL DEFAULT 'pay_at_shop'
                    CHECK (order_type IN ('delivery_cod','pay_at_shop')),
    fulfillment_mode VARCHAR(20) NOT NULL DEFAULT 'shop_direct'
                    CHECK (fulfillment_mode IN ('warehouse_delivery','shop_direct','shop_transfer')),
    fulfill_warehouse_id INT REFERENCES warehouses(id),    -- Kho xuất cho khách
    transfer_source_warehouse_id INT REFERENCES warehouses(id), -- Kho nguồn điều chuyển (nếu có)
    transfer_request_id UUID,                               -- Liên kết phiếu điều chuyển
