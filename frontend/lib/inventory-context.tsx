...
export interface TransferRequest {
  id: string; code: string; date: string; fromWarehouse: string; toWarehouse: string
...
function txTxn(r: any): InventoryTransaction {
  const normalizedType = String(r.type || "").replace(/_/g, "-")
  const safeType: InventoryTransaction["type"] =
    normalizedType === "export" || normalizedType === "transfer-out" || normalizedType === "transfer-in"
      ? normalizedType
      : "import"
  return { id: String(r.id), type: safeType, date: r.date?new Date(r.date).toISOString():"", sku: r.sku||"", productName: r.product_name||r.productName||r.name||"", qty: r.qty||r.quantity||0, cost: r.cost||0, note: r.note||"", warehouse: r.warehouse_name||r.warehouse||"", operator: r.operator||r.created_by_name||r.createdBy||"" }
}
function txTfr(r: any): TransferRequest {
  const normalizedStatus = String(r.status || "pending").replace(/_/g, "-")
  const safeStatus: TransferRequest["status"] =
    normalizedStatus === "approved" || normalizedStatus === "in-transit" || normalizedStatus === "completed" || normalizedStatus === "rejected"
      ? normalizedStatus
      : "pending"
  return { id: String(r.id), code: "", date: r.created_at?new Date(r.created_at).toISOString().split("T")[0]:r.date||"", fromWarehouse: r.from_warehouse_name||r.fromWarehouse||"", toWarehouse: r.to_warehouse_name||r.toWarehouse||"", fromWarehouseId: r.from_warehouse_id, toWarehouseId: r.to_warehouse_id, items: (r.items||[]).map((i: any)=>({sku:i.sku,name:i.name||i.product_name||"",qty:i.qty||i.quantity||0,available:i.available||0})), reason: r.reason||r.note||"", note: r.note||"", status: safeStatus, pickupMethod: r.pickup_method||r.pickupMethod||"employee", createdBy: r.created_by_name||r.createdBy||"", customerName: r.customer_name, customerPhone: r.customer_phone, approvedBy: r.approved_by_name, approvedAt: r.approved_at, completedAt: r.completed_at }
}
...
export function InventoryProvider({ children }: { children: ReactNode }) {
  ...
  const fetchTfr = useCallback(async()=>{if(!canAccess)return;try{const r=await transferApi.getAll();if(r.success&&r.data){const transfers=r.data.map(txTfr);const sorted=[...transfers].sort((a,b)=>(a.date||"").localeCompare(b.date||"")||a.id.localeCompare(b.id));const dc:Record<string,number>={};for(const t of sorted){const d=(t.date||"").replace(/-/g,"");dc[d]=(dc[d]||0)+1;t.code=`DC-${d}-${String(dc[d]).padStart(3,"0")}`}setTransferRequests(transfers)}}catch{}}, [canAccess])
  const fetchPOs = useCallback(async()=>{if(user?.role !== "admin")return;try{const r=await purchaseOrderApi.getAll();if(r.success&&r.data)setPurchaseOrders(r.data.map(txPO))}catch{}}, [user?.role])
  ...
  const importItems = useCallback(async({items,warehouse,note,date,operator}:{items:{sku:string;name:string;qty:number;cost:number}[];warehouse:string;note:string;date:string;operator:string})=>{
    ...
    for(const item of items){
      const r = await inventoryApi.importStock({warehouse_id:wid,sku:item.sku,quantity:item.qty,note:`${note} | ${operator}`})
      if(!r.success) throw new Error(r.message || `Nhập kho thất bại với SKU ${item.sku}`)
    }
  ...
  const createTransfer = useCallback(async(request: Omit<TransferRequest,"id">):Promise<string>=>{
    ...
    const r=await transferApi.create({
      from_warehouse_id:fid,
      to_warehouse_id:tid,
      reason: request.reason || "Điều chuyển kho",
      note: request.note || "",
      pickup_method: request.pickupMethod || "employee",
      customer_name: request.customerName || undefined,
      customer_phone: request.customerPhone || undefined,
      items:request.items.map(i=>({
        sku:i.sku,
        name:i.name,
        qty:i.qty,
        available_at_request:i.available,
      }))
    })
    if(!r.success || !r.data?.id) throw new Error(r.message || "Tạo yêu cầu điều chuyển thất bại")
    await fetchTfr(); return String(r.data.id)
  }, [fetchTfr])

  const updateTransferStatus = useCallback(async(id:string,status:TransferRequest["status"])=>{
    const dbStatus = status.replace(/-/g,"_")
    const r = await transferApi.updateStatus(id,dbStatus)
    if(!r.success) throw new Error(r.message || "Cập nhật trạng thái phiếu điều chuyển thất bại")
    await fetchTfr()
  }, [fetchTfr])
  ...
  const exportTransferItems = useCallback(async({transferId,qtys,date,note,operator}:{transferId:string;qtys:Record<string,number>;date:string;note:string;operator:string})=>{
    ...
    for(const item of tf.items){
      const eq=qtys[item.sku]||0
      if(eq<=0)continue
      const r = await inventoryApi.exportStock({warehouse_id:fid,sku:item.sku,quantity:eq,note:`XKDC ${transferId} | ${note} | ${operator}`})
      if(!r.success) throw new Error(r.message || `Xuất điều chuyển thất bại với SKU ${item.sku}`)
    }
    const statusUpdate = await transferApi.updateStatus(transferId,"in_transit")
    if(!statusUpdate.success) throw new Error(statusUpdate.message || "Không thể cập nhật trạng thái phiếu điều chuyển")
  ...
  const receiveTransferItems = useCallback(async(transferId:string,operator:string)=>{
    ...
    for(const item of tf.items){
      const r = await inventoryApi.importStock({warehouse_id:tid,sku:item.sku,quantity:item.qty,note:`NKDC ${transferId} | ${operator}`})
      if(!r.success) throw new Error(r.message || `Nhận điều chuyển thất bại với SKU ${item.sku}`)
    }
    const statusUpdate = await transferApi.updateStatus(transferId,"completed")
    if(!statusUpdate.success) throw new Error(statusUpdate.message || "Không thể cập nhật trạng thái hoàn thành")
    await Promise.all([fetchInv(),fetchTxn(),fetchTfr()])
  }, [transferRequests,fetchInv,fetchTxn,fetchTfr])
  ...
  const createAdminSlip = useCallback((slip: Omit<AdminWarehouseSlip,"id">)=>{
    ...
    const sameDayCount = adminSlips.filter((s) => String(s.id).startsWith(`${pfx}-${datePart}-`)).length + 1
    const id=`${pfx}-${datePart}-${String(sameDayCount).padStart(3,"0")}`
