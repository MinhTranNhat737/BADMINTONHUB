...
import { printTransferSlip, printWarehouseSlip, printOrderInvoice } from "@/lib/print-utils"
...
import {
  ...
  XCircle, Filter, Download, Printer,
} from "lucide-react"
...
interface SalesOrder {
  id: string; displayCode: string; items: OrderItem[]; customer: { name: string; phone: string; email: string; address: string }
...
export default function AdminAllOrdersPage() {
      ...
      try {
        ...
        if (soRes.success && soRes.data) {
          setSalesOrders(soRes.data.map((o: any) => ({
            id: String(o.id), displayCode: formatHDReference(o.sales_code || o.id, o.created_at), items: (o.items || []).map((i: any) => ({ productId: i.product_id, name: i.product_name || i.name || "", price: i.price || 0, qty: i.qty || i.quantity || 0 })),
        ...
        if (orRes.orders && orRes.orders.length > 0) {
          setSalesOrders(prev => {
            ...
            const newOrders = orRes.orders.filter((o: any) => !existingIds.has(String(o.id))).map((o: any) => ({
              id: String(o.id), displayCode: formatHDReference(o.orderCode || o.order_code || o.code || o.id, o.createdAt || o.created_at), items: (o.items || []).map((i: any) => ({ productId: i.productId || i.product_id, name: i.productName || i.name || "", price: i.price || 0, qty: i.quantity || i.qty || 0 })),
  ...
  function handlePrintRow(row: UnifiedOrder) {
    if (row.type === "transfer") {
      printTransferSlip({
        code: row.raw?.code || row.id,
        date: row.date,
        fromWarehouse: row.raw?.fromWarehouse || "",
        toWarehouse: row.raw?.toWarehouse || "",
        reason: row.raw?.reason || "",
        note: row.raw?.note || "",
        status: row.status,
        createdBy: row.raw?.createdBy || row.operator,
        pickupMethod: row.raw?.pickupMethod || "",
        items: (row.raw?.items || []).map((item: any) => ({
          sku: item.sku,
          name: item.name,
          qty: item.qty,
        })),
      })
    } else if (row.type === "import" || row.type === "export") {
      const items = Array.isArray(row.raw?.items) && row.raw.items.length > 0
        ...
        : [{
            ...
            unitCost: Number(row.raw?.cost ?? 0),
          }]
      printWarehouseSlip({
        id: row.id,
        type: row.type,
      ...
      printOrderInvoice({
        ...
        customerName: row.raw?.customer?.name || "Khách",
        customerPhone: row.raw?.customer?.phone,
        ...
        subtotal: row.raw?.subtotal || row.value,
        discount: 0,
        total: row.raw?.total ?? row.value,
      })
    } else if (row.type === "po") {
      const poItems = Array.isArray(row.raw?.items) ? row.raw.items : []
      printWarehouseSlip({
        id: row.id,
        type: "import",
        date: row.date,
                ...
                <TableRow key={`${row.type}-${row.id}`} className={cn("hover:bg-muted/50", idx % 2 !== 0 && "bg-muted/20")}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-0.5">
                      ...
                      <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handlePrintRow(row)}>
...
function OrderDetailSheet({ row }: { row: UnifiedOrder }) {
  ...
  const handlePrintDetail = () => {
    if (row.type === "transfer") {
      ...
      const items = Array.isArray(row.raw?.items) && row.raw.items.length > 0
        ? row.raw.items.map((item: any) => ({
            sku: item.sku, name: item.name || "", qty: Number(item.qty || 0),
            unitCost: Number(item.unitCost ?? item.cost ?? 0),
          }))
        : [{ sku: row.raw?.sku || "", name: row.raw?.productName || row.description,
            qty: Number(row.raw?.qty || row.qty || 0), unitCost: Number(row.raw?.cost ?? 0) }]
      printWarehouseSlip({
        id: row.id, type: row.type, date: row.date, warehouse: row.warehouse,
        supplier: row.raw?.supplier, poId: row.raw?.poId,
        note: row.raw?.note || row.description,
        createdBy: row.raw?.createdBy || row.operator,
        assignedTo: row.raw?.assignedTo || "", processedBy: row.raw?.processedBy, items,
      })
    } else if (row.type === "sale") {
      const saleItems = Array.isArray(row.raw?.items) ? row.raw.items : []
      printOrderInvoice({
        code: row.id, date: row.raw?.createdAt || row.date,
        customerName: row.raw?.customer?.name || "Khách",
        customerPhone: row.raw?.customer?.phone,
        address: row.raw?.customer?.address,
        paymentMethod: row.raw?.paymentMethod, note: row.raw?.note,
        items: saleItems.map((item: any) => ({
          name: item.name || "", sku: item.sku, qty: Number(item.qty || 0), price: Number(item.price || 0),
        })),
        subtotal: row.raw?.subtotal || row.value, discount: 0, total: row.raw?.total ?? row.value,
      })
    } else if (row.type === "po") {
      const poItems = Array.isArray(row.raw?.items) ? row.raw.items : []
      printWarehouseSlip({
        id: row.id, type: "import", date: row.date,
        warehouse: row.raw?.warehouse || row.warehouse,
        supplier: row.raw?.supplier, note: row.raw?.note || "",
        createdBy: row.operator || "", assignedTo: "",
        items: poItems.map((item: any) => ({
          sku: item.sku, name: item.name || "", qty: Number(item.qty || 0),
          unitCost: Number(item.unitCost ?? item.price ?? 0),
        })),
      ...
      <div className="mt-6 space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="h-8 gap-1.5" onClick={handlePrintDetail}>
              <Printer className="h-3.5 w-3.5" />
              In phiếu
            </Button>
            {canExportDoc && (
