...
import {
  ...
  Building2, ClipboardList, Users, QrCode, Search
} from "lucide-react"
...
const navGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/admin/products/import", icon: <FileText className="h-5 w-5" />, label: "Import mẫu mã" },
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/admin/document-audit", icon: <Search className="h-5 w-5" />, label: "Rà soát mã phiếu" },
    ],
  },
  {
    label: "Hệ thống",
...
function AdminTopbar({ collapsed, onMobileMenu }: { collapsed: boolean; onMobileMenu: () => void }) {
  ...
  const breadcrumbMap: Record<string, string> = {
    ...
    '/admin/products/import': 'Import mẫu mã',
    '/admin/purchase-orders': 'Đặt hàng',
    '/admin/document-audit': 'Rà soát mã phiếu',
