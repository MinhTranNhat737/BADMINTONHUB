...
function InvoiceSection({ booking }: { booking: CompletedBooking }) {
  ...
  const displayBookingCode = formatBookingReference(booking.id)
  const now = new Date()
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex justify-between items-start text-sm">
            ...
            <div className="text-right">
              ...
              <p className="font-mono font-bold text-primary">{displayBookingCode}</p>
          ...
          <div className="text-center text-xs text-muted-foreground space-y-1">
            ...
            <p className="font-mono text-[10px] mt-2">Mã giao dịch: TXN-{displayBookingCode}</p>
          </div>
...
export default function BookingSuccessPage() {
  ...
  const displayBookingCode = formatBookingReference(data.id)
        ...
        <div className="mx-auto max-w-lg px-4 w-full">
          ...
          <div className="text-center mt-4 animate-fade-in-up opacity-0 stagger-2" style={{ animationFillMode: 'forwards' }}>
            ...
            <p className="text-muted-foreground mt-2">
              Mã đặt sân của bạn là <span className="font-mono font-bold text-primary">{displayBookingCode}</span>
            </p>
          ...
          <div className="mt-6 flex justify-center animate-fade-in-up opacity-0 stagger-3" style={{ animationFillMode: 'forwards' }}>
            <div className="h-[160px] w-[160px] rounded-xl border-2 border-dashed border-muted-foreground/30 flex items-center justify-center bg-white p-3">
              <div className="text-center">
                <QRCodeSVG
                  value={JSON.stringify({ bookingId: (data as any).bookingId || data.id, bookingCode: displayBookingCode })}
                  size={110}
                  level="M"
                />
