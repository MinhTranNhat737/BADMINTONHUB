...
export default function OrderSuccessPage() {
  ...
  const { user } = useAuth()
  const displayOrderCode = order ? formatHDReference(order.orderCode || order.id, order.createdAt) : ""
  ...
  const handleDownload = async () => {
    if (!order || !displayOrderCode) return
            ...
            {data && (
              <div className="mt-2 flex items-center justify-center gap-2">
                <p className="text-muted-foreground">
                  Mã đơn hàng: <span className="font-mono font-bold text-primary" title={data.rawId || data.id}>{displayOrderCode}</span>
                </p>
                <button
                  onClick={handleCopy}
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex justify-between items-start text-sm">
                    ...
                    <div className="text-right">
                      ...
                      <p className="font-mono font-bold text-primary" title={data.rawId || data.id}>{displayOrderCode}</p>
