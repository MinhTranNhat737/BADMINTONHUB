...
export default function CheckoutPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const resolvedFulfillingWarehouse = deliveryMethod === "pickup"
        ? (selectedBranch ? `Kho ${selectedBranch.name}` : undefined)
        : (customerCoords ? findNearestWarehouse(customerCoords.lat, customerCoords.lng).warehouseName : undefined)

      const result = await orderApi.create({
        type: "online",
        delivery_method: deliveryMethod,
        pickup_branch_id: deliveryMethod === "pickup" ? pickupBranch : undefined,
        fulfilling_warehouse: resolvedFulfillingWarehouse,
      ...
      if (result.success && result.order) {
        const displayOrderCode = formatHDReference(result.order.orderCode || result.order.id, result.order.createdAt)
        // Save for success page display
        localStorage.setItem("badmintonhub_completed_order", JSON.stringify({
          id: displayOrderCode,
          rawId: result.order.id,
          items: cart,
