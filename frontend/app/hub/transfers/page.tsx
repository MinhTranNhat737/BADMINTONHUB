...
import {
  ...
  ArrowDownToLine, FileText, Printer
} from "lucide-react"
...
function safeTransferItems(items: any): { sku: string; name: string; qty: number; available: number }[] {
  if (!Array.isArray(items)) return []
  return items
    .filter((item) => item && typeof item === "object")
    .map((item) => ({
      sku: String(item.sku || ""),
      name: String(item.name || item.product_name || "Sản phẩm"),
      qty: Math.max(0, Number(item.qty || item.quantity || 0)),
      available: Math.max(0, Number(item.available || item.available_at_request || 0)),
    }))
...
export default function HubTransfersPage() {
  ...
  const {
    ...
    importItems, exportItems, processAdminSlip, createAdminSlip
  } = useInventory()
  ...
  // PNK slip ID created after receive
  const [receivePnkId, setReceivePnkId] = useState<string | null>(null)

  const handlePrintTransfer = (t: TransferRequest) => {
    const items = safeTransferItems(t.items)
    printTransferSlip({
      code: t.code, date: t.date, fromWarehouse: t.fromWarehouse, toWarehouse: t.toWarehouse,
      reason: t.reason, note: t.note, status: t.status, createdBy: t.createdBy, pickupMethod: t.pickupMethod,
      items: items.map(i => ({ sku: i.sku, name: i.name, qty: i.qty })),
    })
  }

  const handlePrintSlip = (s: AdminWarehouseSlip) => {
    printWarehouseSlip({
      id: s.id, type: s.type, date: s.date, warehouse: s.warehouse,
      supplier: s.supplier, poId: s.poId, note: s.note, createdBy: s.createdBy,
      assignedTo: s.assignedTo, processedBy: s.processedBy,
      items: s.items.map(i => ({ sku: i.sku, name: i.name, qty: i.qty, unitCost: i.unitCost })),
    })
  }
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      if (actionType === "approve") {
        ...
        safeTransferItems(actionTransfer.items).forEach(i => { qtys[i.sku] = i.qty })
        ...
        // Auto-create PNK slip for the receiving warehouse
        const pnkId = createAdminSlip({
          type: "import",
          source: "admin",
          date: today,
          warehouse: actionTransfer.toWarehouse,
          items: safeTransferItems(actionTransfer.items).map(i => ({ sku: i.sku, name: i.name, qty: i.qty, unitCost: 0 })),
          note: `Nhận điều chuyển ${actionTransfer.code} | Từ ${actionTransfer.fromWarehouse}`,
          status: "processed",
          createdBy: user?.fullName || "NV Hub",
          assignedTo: actionTransfer.toWarehouse,
          processedAt: new Date().toISOString(),
          processedBy: user?.fullName || "NV Hub",
        })
        setReceivePnkId(pnkId)
      } else if (actionType === "reject") {
        await updateTransferStatus(actionTransfer.id, "rejected")
      }
      setActionSuccess(true)
      setTimeout(() => { setActionSuccess(false); setActionOpen(false); setReceivePnkId(null) }, actionType === "receive" ? 5000 : 1200)
    } catch (error) {
        ...
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              ...
              {filtered.map(t => (
                <TableRow key={t.id}>
                  <TableCell className="font-mono text-xs text-purple-600">{t.code}</TableCell>
                  ...
                  <TableCell className="text-sm max-w-[180px] truncate">
                    {safeTransferItems(t.items).map(i => `${i.name} (x${i.qty})`).join(", ")}
                  </TableCell>
            ...
            <div className="flex flex-col items-center gap-3 py-6">
              ...
              {receivePnkId && (
                <div className="text-center space-y-2">
                  <p className="text-sm text-muted-foreground">Đã tạo phiếu nhập kho: <span className="font-mono font-medium text-primary">{receivePnkId}</span></p>
                  <Button size="sm" variant="outline" onClick={() => {
                    const slip = adminSlips.find(s => s.id === receivePnkId)
                    if (slip) handlePrintSlip(slip)
                  }}>
                    <Printer className="h-3.5 w-3.5 mr-1" /> In phiếu nhập kho
                  </Button>
                </div>
              )}
              {actionTransfer && actionType !== "receive" && (
                <Button size="sm" variant="outline" onClick={() => handlePrintTransfer(actionTransfer)}>
                  <Printer className="h-3.5 w-3.5 mr-1" /> In phiếu điều chuyển
                </Button>
              )}
            </div>
          ) : actionTransfer && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <p className="font-mono font-medium text-purple-600">{actionTransfer.code}</p>
                </div>
                <div>
              ...
              <Table>
                ...
                <TableBody>
                  {safeTransferItems(actionTransfer.items).map(item => (
                    <TableRow key={item.sku}>
          ...
          {slipSuccess ? (
            ...
            <div className="space-y-4">
              ...
              <DialogFooter>
                ...
                <Button variant="outline" onClick={() => handlePrintSlip(selectedSlip)}>
        ...
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="font-serif flex items-center gap-2">
              ...
              Chi tiết điều chuyển {selectedTransfer ? selectedTransfer.code : ""}
            </DialogTitle>
          </DialogHeader>
          {selectedTransfer && (
            <div className="space-y-4">
              ...
              <Table>
                ...
                <TableBody>
                  {safeTransferItems(selectedTransfer.items).map(item => (
                    <TableRow key={item.sku}>
                      <TableCell className="font-mono text-xs text-primary">{item.sku}</TableCell>
                      <TableCell className="text-sm">{item.name}</TableCell>
                      <TableCell className="text-center font-medium">{item.qty}</TableCell>
          ...
          <DialogFooter>
            ...
            {selectedTransfer && (
              <Button variant="outline" onClick={() => handlePrintTransfer(selectedTransfer)}>
                <Printer className="h-4 w-4 mr-1" /> In phiếu
              </Button>
            )}
          </DialogFooter>
