...
import { Menu, X, ShoppingCart, User, Phone, MapPin, LogOut, Shield, LogIn } from "lucide-react"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
    ...
    <>
      ...
      >
        <div className="mx-auto max-w-7xl flex items-center justify-between px-6 h-20">
          ...
          <div className="hidden md:flex items-center gap-2">
            <Link href="/shop">
              <Button
                variant="ghost"
                size="icon"
                className="relative h-9 w-9 rounded-lg text-gray-600 hover:text-[#0A2416] hover:bg-gray-100"
              >
                <ShoppingCart className="h-5 w-5" />
                {totalItems > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 flex h-4.5 w-4.5 items-center justify-center rounded-full bg-primary text-[10px] font-bold text-white shadow-sm">
                    {totalItems}
                  </span>
                )}
              </Button>
            ...
            {user ? (
              user.role === "guest" ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Link href="/login" onClick={() => logout()}>
                    <Button variant="outline" size="sm" className="h-8 text-xs font-semibold rounded-md border-[#1F6B3A] text-[#1F6B3A] hover:bg-[#1F6B3A]/5">
                      <LogIn className="h-3.5 w-3.5 mr-1" />
                      Đăng nhập
                    </Button>
                  </Link>
                  <Link href="/register" onClick={() => logout()}>
                    <Button size="sm" className="h-8 bg-[#1F6B3A] text-white hover:bg-[#185a30] text-xs font-semibold rounded-md">
                      Đăng ký
                    </Button>
                  </Link>
                </div>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="ghost"
                    className="h-9 rounded-lg text-gray-600 hover:text-[#0A2416] hover:bg-gray-100 gap-1.5 px-2"
                  >
              ...
              <Link href="/login">
                <Button
                  variant="ghost"
                  className="h-9 rounded-lg text-gray-600 hover:text-[#0A2416] hover:bg-gray-100 gap-1.5 px-3"
                >
          ...
          {/* Mobile Menu Toggle */}
          <button
            className="md:hidden flex items-center justify-center h-11 w-11 rounded-xl text-gray-600 hover:bg-gray-100 transition-colors"
          ...
          <div className="px-4 pb-5 pt-3">
            ...
            <div className="flex gap-2 mt-4 pt-4 border-t border-gray-100">
              <Link href="/courts" className="flex-1" onClick={() => setMobileOpen(false)}>
                <Button className="w-full bg-[#1F6B3A] text-white hover:bg-[#185a30] font-semibold rounded-xl shadow-md">
                  🏸 Đặt sân ngay
                </Button>
              </Link>
            </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2 px-4">
                    <Link href="/login" className="flex-1" onClick={() => { logout(); setMobileOpen(false) }}>
                      <Button variant="outline" className="w-full rounded-xl font-semibold border-[#1F6B3A] text-[#1F6B3A]">
                        Đăng nhập
                      </Button>
                    </Link>
                    <Link href="/register" className="flex-1" onClick={() => { logout(); setMobileOpen(false) }}>
                      <Button className="w-full bg-[#1F6B3A] text-white hover:bg-[#185a30] rounded-xl font-semibold">
                        Đăng ký
                      </Button>
                    </Link>
                  </div>
                ...
                <>
                  ...
                  {user.role === "admin" && (
                    <Link
                      href="/admin"
                      onClick={() => setMobileOpen(false)}
                      className="flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-xl text-gray-600 hover:text-[#0A2416] hover:bg-gray-50"
                    >
                  ...
                  <button
                    ...
                    className="flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-xl text-red-600 hover:bg-red-50"
                  >
