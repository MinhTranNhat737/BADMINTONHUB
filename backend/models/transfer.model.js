...
const Transfer = {
  ...
  create: async (data) => {
    ...
    try {
      ...
      const sql = `INSERT INTO transfer_requests (date, from_warehouse_id, to_warehouse_id, reason, note,
                   ...
                   VALUES (NOW(), $1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`;
  ...
  updateStatus: async (id, status, userId) => {
    ...
    if (status === 'approved') {
      ...
      sql = `UPDATE transfer_requests SET status = $1, approved_by = $2 WHERE id = $3 RETURNING *`;
    }
    const result = await query(sql, [status, userId, id]);
